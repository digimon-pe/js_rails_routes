...
RSpec.describe JSRailsRoutes::Language::JavaScript do
  ...
  describe '#handle_route_set' do
    context 'without prefex params' do
    ...
    context 'with prefix params' do
      subject { language.handle_route_set(route_set_with_prefix_params) }

      let(:route_set_with_prefix_params) do
        rails_route_set = ActionDispatch::Routing::RouteSet.new.tap do |routes|
          routes.draw do
      ...
      context 'without camelize option' do
        it 'returns a javascript with snake_case functions' do
          expect(subject).to eq <<~JAVASCRIPT
            #{described_class::PROCESS_FUNC}
            export function articles_path(params) { return process('' + params.locale ? '/' + params.locale : '' + '/articles', params, ['locale']); }
            export function new_article_path(params) { return process('' + params.locale ? '/' + params.locale : '' + '/articles/new', params, ['locale']); }
            export function edit_article_path(params) { return process('' + params.locale ? '/' + params.locale : '' + '/articles/' + params.id + '/edit', params, ['locale','id']); }
            export function article_path(params) { return process('' + params.locale ? '/' + params.locale : '' + '/articles/' + params.id + '', params, ['locale','id']); }
          JAVASCRIPT
        end
      end
      ...
      context 'with camelize = :lower option' do
        ...
        context 'when without params' do
          it 'returns a javascript with lowerCamelCase functions' do
            expect(subject).to eq <<~JAVASCRIPT
              #{described_class::PROCESS_FUNC}
              export function articlesPath(params) { return process('' + params.locale ? '/' + params.locale : '' + '/articles', params, ['locale']); }
              export function newArticlePath(params) { return process('' + params.locale ? '/' + params.locale : '' + '/articles/new', params, ['locale']); }
              export function editArticlePath(params) { return process('' + params.locale ? '/' + params.locale : '' + '/articles/' + params.id + '/edit', params, ['locale','id']); }
              export function articlePath(params) { return process('' + params.locale ? '/' + params.locale : '' + '/articles/' + params.id + '', params, ['locale','id']); }
            JAVASCRIPT
          end
        end
      end
